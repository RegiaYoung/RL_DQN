...
class Agent(object):
    ...
    def random_learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        actions_next = None
        with torch.no_grad():
            actions_next = self.__policy(next_batch.float()).max(1)[1]
        actions_next = actions_next.reshape(-1,1)
        values_next = self.__target(next_batch.float())
       
        values_next = values_next.gather(1,actions_next)
