...
class ReplayMemory(object):
    ...
    def prior_sample(self, batch_size: int ,weight) -> Tuple[
            BatchState,
            BatchAction,
            BatchReward,
            BatchNext,
    ...
    def full_sample(self) -> Tuple[
        ...
        indices = torch.arange(self.__size)

        b_state = self.__m_states[:, :4].to(self.__device).float()
        b_next = self.__m_states[:, 1:].to(self.__device).float()
        b_action = self.__m_actions[:].to(self.__device)
        b_reward = self.__m_rewards[:].to(self.__device).float()
        b_done = self.__m_dones[:].to(self.__device).float()
        return b_state, b_action, b_reward, b_next, b_done

    def index_sample(self, indices) -> Tuple[
            BatchState,
            BatchAction,
            BatchReward,
            BatchNext,
